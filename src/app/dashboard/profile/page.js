        ...
        return (
            <div className="space-y-6 font-figtree animate-in fade-in duration-500 max-w-[1400px] mx-auto pb-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 h-96 bg-card-bg border border-pace-border rounded-2xl animate-pulse" />
                </div>
            </div>
        )
    }
        ...
        <div className="space-y-6 font-figtree animate-in fade-in duration-500 max-w-[1400px] mx-auto pb-10">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-pace-border pb-6">
                ...
                <button
                    onClick={loadProfile}
                    disabled={isLoading}
                    className="inline-flex items-center gap-2 px-3 py-2 bg-pace-bg-subtle text-admin-dim border border-pace-border rounded-xl hover:bg-pace-purple/5 hover:text-pace-purple transition-all text-xs font-semibold cursor-pointer shrink-0 self-start sm:self-auto"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                    <div className="bg-card-bg border border-pace-border rounded-2xl p-6 shadow-sm relative overflow-hidden">
                        ...
                        <div className="flex flex-col items-center text-center pt-2">
                            <div className="w-20 h-20 rounded-2xl bg-pace-purple/10 border border-pace-purple/20 flex items-center justify-center text-pace-purple text-2xl font-bold tracking-wider mb-4 shadow-sm">
                                {getInitials(name || profile?.name)}
                            </div>
                            
                            <h2 className="text-base font-bold text-admin-value">{name || profile?.name || 'Administrator'}</h2>
                            <p className="text-xs font-mono font-medium text-admin-dim mt-0.5">@{profile?.username || 'user'}</p>
                            ...
                            <div className="w-full space-y-4 text-left text-xs">
                                {profile?.role === 'isp' && profile?.wallet_balance !== undefined && (
                                    <div className="flex items-center justify-between p-3 bg-pace-bg-subtle/70 rounded-xl border border-pace-border/60">
                                        <div className="flex items-center gap-2.5 text-admin-dim">
                                            <Wallet size={15} className="text-pace-purple" />
                                            <span className="font-semibold">Wallet Balance</span>
                                        </div>
                                        <span className="font-mono font-bold text-admin-value text-sm">
                                            KES {Number(profile.wallet_balance).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                                        </span>
                                    </div>
                                )}

                                <div className="flex justify-between py-1 border-b border-pace-border/40">
                                ...
                                <div className="flex justify-between py-1 border-b border-pace-border/40">
                                    ...
                                    <span className="font-medium text-admin-value">{profile?.phone || 'Not set'}</span>
                                </div>

                                <div className="flex justify-between py-1 border-b border-pace-border/40">
                                    <span className="text-admin-dim font-medium">Email Address</span>
                                    <span className="font-medium text-admin-value">{profile?.email || 'Not set'}</span>
                                </div>

                                <div className="flex justify-between py-1">
                                    <span className="text-admin-dim font-medium">Member Since</span>
                                    <span className="font-medium text-admin-value">
                ...
                {/* Right Area: Forms (Identity Settings + Change Password) */}
                    ...
                    <div className="bg-card-bg border border-pace-border rounded-2xl p-6 sm:p-8 shadow-sm">
                        ...
                        <form onSubmit={handleProfileSubmit} className="space-y-5">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            placeholder="e.g. 0712345678 / +254..."
                                            className="w-full pl-10 pr-4 py-2.5 bg-pace-bg-subtle border border-pace-border rounded-xl text-xs font-medium text-admin-value focus:outline-none focus:border-pace-purple transition-all"
                                        />
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            placeholder="e.g. dennis@pacesystem.com"
                                            className="w-full pl-10 pr-4 py-2.5 bg-pace-bg-subtle border border-pace-border rounded-xl text-xs font-medium text-admin-value focus:outline-none focus:border-pace-purple transition-all"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[11px] font-bold text-admin-dim uppercase tracking-wider">System Username (Locked)</label>
                                    <div className="relative">
                    ...
                    <div className="bg-card-bg border border-pace-border rounded-2xl p-6 sm:p-8 shadow-sm">
                        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-pace-border">
                            ...
                            <div>
                                ...
                                <p className="text-[11px] text-admin-dim font-medium">Update your account authentication credentials.</p>
                            </div>
                        </div>
